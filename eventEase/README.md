...
- **JOIN System**: Users can JOIN approved events with capacity management
- **Dashboard**: Separate dashboards for admins and users

### User Roles
- **Admin**: Full system access, event approval, user management
- **Teacher**: Create events, JOIN events, manage own events
- **Student**: Create events, JOIN events, manage own events
...
4. **Browse Events**: JOIN approved events
5. **Track JOINs**: See events you're attending
...
- `POST /user/rsvp.php` - JOIN events
